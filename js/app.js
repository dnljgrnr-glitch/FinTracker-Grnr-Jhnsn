...
init();
  ...
  $('#importBtn')?.addEventListener('click', () => {
    ...
    reader.onload = e => {
      try {
        const imp = JSON.parse(e.target.result); let n = 0;
        Object.entries(imp).forEach(([k,v]) => {
          if (['balancelog','lastUpdated'].includes(k) || ['goal:','gs:','tracker:','checks:','spend:','slider:','notes:'].some(p => k.startsWith(p))) { LS.set(k,v); n++; }
        });
        stat.textContent = `✅ Imported ${n} items. Reloading…`; stat.style.color = 'var(--good-light)';
        setTimeout(() => location.reload(), 1200);
      } catch { stat.textContent = '❌ Invalid file.'; stat.style.color = 'var(--bad-light)'; }
  ...
  $('#logEntryBtn')?.addEventListener('click', () => {
    const entry = {
      dt: $('#log-dt').value,
      dv:   parseFloat($('#log-dv').value)||0,   dveh: parseFloat($('#log-dveh').value)||0,
      da:   parseFloat($('#log-da').value)||0,   sn:   parseFloat($('#log-sn').value)||0,
      sf:   parseFloat($('#log-sf').value)||0,   ss:   parseFloat($('#log-ss').value)||0,
      ds:   parseFloat($('#log-ds').value)||0,
    };
    // Sync goal values
    setGoal('daniel-visa-balance',    entry.dv);   setGoal('daniel-vehicle-balance', entry.dveh);
    setGoal('daniel-apple-balance',   entry.da);   setGoal('sonia-nw-balance',        entry.sn);
    setGoal('sonia-family-balance',   entry.sf);   setGoal('sonia-savings-balance',   entry.ss);
    setGoal('daniel-savings-balance', entry.ds);
    const log = LS.get('balancelog', []); log.push(entry); LS.set('balancelog', log);
    touchLastUpdated(); showToast('Snapshot saved ✓');
    const s = $('#logStatus'); if (s) { s.textContent = `✅ Saved for ${entry.dt}`; s.style.color = 'var(--good-light)'; }
    renderAll(); setTimeout(() => renderSettings(), 300);
  });
...
function initSubTabs() {
  document.addEventListener('click', e => {
    ...
    $$(`#${btn.closest('section,main').id || 'main-content'} .sub-panel`).forEach(p => p.classList.remove('active'));
    btn.classList.add('active'); btn.setAttribute('aria-selected','true');
    // find the panel in the nearest parent section
    const panel = document.getElementById(`sp-${group}-${id}`);
...
function initEvents() {
  ...
  document.addEventListener('change', e => {
    ...
    /* Goal inputs (Goals Editor + caps + bill split) */
    if (e.target.matches('input[data-goal]')) {
      const key = e.target.dataset.goal;
      const val = parseFloat(e.target.value);
      if (!isNaN(val)) { setGoal(key, val); touchLastUpdated(); showToast('Saved ✓'); renderAll(); }
    ...
    if (e.target.matches('input[data-spend]')) {
      const [person, cat] = e.target.dataset.spend.split(':');
      const val = parseFloat(e.target.value);
      if (!isNaN(val) && val >= 0) { setSpent(person, cat, val); touchLastUpdated(); showToast('Spending updated ✓'); renderAll(); }
    }
    ...
    if (['calc-monthly','calc-rate','calc-years'].includes(e.target.id)) {
      LS.set(e.target.id, parseFloat(e.target.value));
      // Re-render just the investing sub-panel
      renderPlanning();
    }
  });
  ...
  document.addEventListener('input', e => {
    if (e.target.matches('input[data-gs-input]')) {
      ...
      const wrap    = e.target.closest('[data-gs]');
      const target  = parseFloat(wrap?.dataset.gsTarget || 0);
      const type    = wrap?.dataset.gsType || 'debt';
      const pct     = target > 0 ? Math.min(100, val/target*100) : 0;
      const remaining = Math.max(0, target - val);
      ...
      const fillEl = $(`#gsf-${id}`);    if (fillEl) fillEl.style.width  = `${pct}%`;
      const numEl  = $(`#gsn-${id}`);    if (numEl)  numEl.textContent   = money(val);
      // Update the two stat values inside the same wrap
      const statVals = $$('.goal-stat-val', wrap);
      if (statVals[0]) statVals[0].textContent = money(val);
      if (statVals[1]) statVals[1].textContent = money(remaining);
    }
    ...
    if (e.target.matches('input[type="range"][data-slider]')) {
      ...
      const tM = totalInterestPaid(bal, apr, min);
      const sv = Math.max(0, (isFinite(tM)?tM:0) - (isFinite(ti)?ti:0));
      $(`#sv-${id}`)?.textContent  === undefined ? null : ($(`#sv-${id}`).textContent = money(pmt));
      const svEl = $(`#sv-${id}`); if (svEl) svEl.textContent = money(pmt);
      const mEl  = $(`#pom-${id}`); if (mEl)  mEl.textContent  = isFinite(n)?n:'∞';
      const iEl  = $(`#poi-${id}`); if (iEl)  iEl.textContent  = isFinite(ti)?money(Math.round(ti)):'∞';
      const vEl  = $(`#pov-${id}`); if (vEl)  vEl.innerHTML    = `vs. minimum: ${isFinite(nM)?nM:'∞'} months · <strong class="private-value">save ${money(Math.round(sv))}</strong>`;
    }
  });
...
function refreshLastUpdated() {
  const el = $('#lastUpdatedDisplay'); if (!el) return;
  const ts = LS.get('lastUpdated');
  el.textContent = ts ? `Saved ${new Date(ts).toLocaleDateString()} ${new Date(ts).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}` : '';
}
function touchLastUpdated() { LS.set('lastUpdated', new Date().toISOString()); refreshLastUpdated(); }
...
function initPrivacyToggle() {
  ...
  applyPrivacy(on, btn);
  btn.addEventListener('click', () => { const n = !document.body.classList.contains('privacy-mode'); LS.set('privacyMode',n); applyPrivacy(n,btn); });
}
function applyPrivacy(on, btn) {
  document.body.classList.toggle('privacy-mode', on);
  btn.setAttribute('aria-pressed', String(on));
  const l = btn.querySelector('.btn-label'); if (l) l.textContent = on ? 'Show $' : 'Hide $';
}
...
function init() {
  auth();
  renderDashboard(); renderDaniel(); renderSonia();
  renderHousehold(); renderPlanning(); renderSettings();
  initTabs(); initSubTabs(); initEvents(); initPrivacyToggle();
  refreshLastUpdated();
}
...
function initEvents() {
  ...
  document.addEventListener('change', e => {
    ...
    if (e.target.matches('input[type="checkbox"][data-list]')) {
      ...
      e.target.closest('.check-row').classList.toggle('done', e.target.checked);
      touchLastUpdated();
      const boxes = Array.from(document.querySelectorAll(`input[data-list="${list}"]`));
      const done  = boxes.filter(b => b.checked).length;
  ...
  document.addEventListener('input', e => {
    ...
    if (e.target.matches('input[data-gs-input]')) {
      ...
      const val     = parseFloat(e.target.value);
      const wrap    = e.target.closest('[data-gs]');
      const target  = parseFloat(wrap ? wrap.dataset.gsTarget : 0);
      LS.set(`gs:${id}`, val);
      touchLastUpdated();
      const pct       = target > 0 ? Math.min(100, val / target * 100) : 0;
      const remaining = Math.max(0, target - val);
      const pctEl  = document.getElementById(`gsp-${id}`); if (pctEl)  pctEl.textContent  = `${pct.toFixed(0)}%`;
      const fillEl = document.getElementById(`gsf-${id}`); if (fillEl) fillEl.style.width  = `${pct}%`;
      const numEl  = document.getElementById(`gsn-${id}`); if (numEl)  numEl.textContent   = money(val);
      if (wrap) {
        const stats = Array.from(wrap.querySelectorAll('.goal-stat-val'));
        if (stats[0]) stats[0].textContent = money(val);
        if (stats[1]) stats[1].textContent = money(remaining);
      }
    ...
    if (e.target.matches('input[type="range"][data-slider]')) {
      ...
      const tM = totalInterestPaid(bal, apr, min);
      const sv = Math.max(0, (isFinite(tM) ? tM : 0) - (isFinite(ti) ? ti : 0));
      const svEl = document.getElementById(`sv-${id}`);  if (svEl) svEl.textContent = money(pmt);
      const mEl  = document.getElementById(`pom-${id}`); if (mEl)  mEl.textContent  = isFinite(n)  ? n  : '∞';
      const iEl  = document.getElementById(`poi-${id}`); if (iEl)  iEl.textContent  = isFinite(ti) ? money(Math.round(ti)) : '∞';
      const vEl  = document.getElementById(`pov-${id}`);
      if (vEl) vEl.innerHTML = `vs. minimum: ${isFinite(nM)?nM:'∞'} months · <strong class="private-value">save ${money(Math.round(sv))}</strong>`;
    }
  });
...
function initPrivacyToggle() {
  ...
  btn.addEventListener('click', () => {
    const next = !document.body.classList.contains('privacy-mode');
    LS.set('privacyMode', next); applyPrivacy(next, btn);
  });
}
function applyPrivacy(on, btn) {
  document.body.classList.toggle('privacy-mode', on);
